...
import { Causes } from "@/types/index";
import { ref, watch, PropType } from "vue";
import Flasher from "@/helprs";
import { FlasherResponse } from "@flasher/flasher";
import CreateCause from "./CreateCause.vue";
import EditCause from "./EditCause.vue";
import DeleteCause from "./DeleteCause.vue";

const props = defineProps({
    causes: {
        type: Object as PropType<Causes>,
        required: true,
    },
    search: String,
...
const links = ref(props.causes.links);

const editingCauseTrigger = ref(false);
const editingCause = ref<App.Data.CauseData | null>(null);

const deletingCauseTrigger = ref(false);
const deletingCause = ref<App.Data.CauseData | null>(null);
...
function openEditCauseModal(cause: App.Data.CauseData) {
    editingCause.value = cause;
    editingCauseTrigger.value = true;
}

function closeEditCauseModal() {
    editingCause.value = null;
    editingCauseTrigger.value = false;
}

function openDeleteCauseModal(cause: App.Data.CauseData) {
    deletingCause.value = cause;
    deletingCauseTrigger.value = true;
}

function closeDeleteCauseModal() {
    deletingCause.value = null;
    deletingCauseTrigger.value = false;
}
</script>
    ...
    <AuthenticatedLayout>
        <template v-slot:content>
                        ...
                        >
                            ...
                            <tbody>
                                <tr
                                    ...
                                    v-for="(cause, index) in causes.data"
                                    :key="cause.id ?? index"
                                >
                                    <th
                                        scope="row"
                                    ...
                                    <td class="px-4 py-3 w-32">
                                        <button
                                            type="button"
                                            @click="openEditCauseModal(cause)"
                                    ...
                                    <td class="px-4 py-3 justify-end w-32">
                                        <button
                                            type="button"
                                            @click="openDeleteCauseModal(cause)"
            ...
            <EditCause
                v-if="editingCause"
                :cause="editingCause"
                :openModal="editingCauseTrigger"
                :close="closeEditCauseModal"
            />
            <DeleteCause
                v-if="deletingCause"
                :cause="deletingCause"
                :openModal="deletingCauseTrigger"
                :close="closeDeleteCauseModal"
            />
        </template>
    </AuthenticatedLayout>
</template>
