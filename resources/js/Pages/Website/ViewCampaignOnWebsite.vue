...
const comment = () => {
    formComment.post(route("campaign.comment",{
...
<template>
    ...
    <section class="bg-gradient-to-b from-emerald-50/10 via-emerald-100 to-gray-100">

        <div class="max-w-screen-lg mx-auto p-4 md:p-8 xl:p-16  h-full">
            <main class="  dark:bg-gray-900 antialiased mx-auto shadow-2xl shadow-emerald-100 "
            :class="{
                    ...
                    <article class="mx-auto w-full  format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
                        <figure>
                            ...
                            <video
                                ...
                                controls
                            ></video>
                            <div class="w-full bg-emerald-100  h-2.5  dark:bg-gray-700 flex items-center">
                                <div class="bg-green-600 h-2.5  rounded-r-none dark:bg-green-500  " :style="`width: ${percentageOfGoal(campaign.goalAmount ,campaign.currentAmount )}`" ></div>
                        ...
                        <div class="p-12 flex flex-col ">
                            <header class="not-format flex justify-between gap-x-4">

                             <div>
                                 <h1 class="mb-4 text-xl font-extrabold font-['Lexend_Deca']   leading-tight text-emerald-900 lg:mb-6 lg:text-2xl dark:text-white">
                                     {{ campaign?.title }}
                                 </h1>
                             </div>

                                <div>
                                    <button @click="openDonationModal(campaign)" class="
                           m-t  p-1.5 px-3 h-fit !w-fit bg-emerald-800 hover:bg-emerald-700 text-white text-sm rounded inline-flex items-center font-['Lexend_Deca']  justify-center gap-x-2 font-black ">
                            ...
                            <div class="inline-flex gap-x-6 mb-4 ">
                                    <span class="bg-gray-500 text-white font-['Lexend_Deca'] p-1 font-black py-1.5 rounded-sm px-3 ">
                                        <span class=" font-normal text-emerald-50 ">Causa</span>:  {{ campaign.cause.title }}
                                    </span>
                                <span class="bg-blue-600 text-white font-['Lexend_Deca'] p-1 font-black py-1.5 rounded-sm px-3 ">
                                        <span class=" font-normal text-emerald-50 ">Meta</span>:  {{ numberMoneyFormat(campaign.goalAmount) }}
                                    </span>
                                <span class="bg-green-500 text-white font-['Lexend_Deca'] p-1 font-black py-1.5 rounded-sm px-3 ">
                                        <span class=" font-normal text-emerald-50 ">Arrecadado</span>:  {{ numberMoneyFormat(campaign.currentAmount ?? 0) }}
                                    </span>
                            </div>
                            <div class="prose !max-w-full" v-html="campaign?.description"></div>
                            <section class="not-format my-5">
                                <div class="flex justify-between items-center mb-6">
                                    <h2 class="text-lg lg:text-xl font-bold font-['Lexend_Deca'] text-emerald-900 dark:text-white">Comentários : {{ countComments }} </h2>
                                </div>
                                <div class="mb-2 flex flex-col space-y-4" >
                                    <div>
                                        <label for="name" class="sr-only">Teu nome</label>
                                        <input id="name"
                                    ...
                                    <div class="py-2 px-4 mb-1 bg-white rounded-lg rounded-t-lg border ring ring-emerald-900  border-emerald-200
                                    flex flex-col
                                     ">
                                        <label for="comment" class="sr-only">Teu comentário</label>
                                        <textarea id="comment" rows="6"
                                                  v-model="formComment.comment"
                                                  class="px-0 w-full
                                                  text-sm text-gray-900 border-0 focus:ring-0"
                                                  placeholder="Comentar.." required></textarea>
                                    </div>
                                    <button type="button"
                                            @click="comment"
                                            class="inline-flex self-end items-center py-2.5 px-4 my-3 text-xs font-medium text-center text-white bg-emerald-700 rounded
                                             focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-emerald-800">
                                        Enviar comentário
                                    </button>
                                </div>
                                <article v-for="(comment,index) in (campaign.comments as CampaignComment[])"  :key="index" class="p-6 mb-4 text-base bg-white border-t border-gray-200 dark:border-gray-700 dark:bg-gray-900">
                                    <footer class="flex justify-between items-center mb-2">
                                        <div class="flex items-center">
                                            <p class="inline-flex items-center mr-3 font-semibold font-['Lexend_Deca'] text-sm text-emerald-900 dark:text-white"><img
                                                class="mr-2 w-6 h-6 rounded-full"
                                                src="https://placehold.co/100x100"
                                                alt="Bonnie Green">{{ comment.name }}</p>
                                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                                <time pubdate datetime="2022-03-12" title="March 12th, 2022">{{ comment.createdAt }}</time></p>
                                        </div>
